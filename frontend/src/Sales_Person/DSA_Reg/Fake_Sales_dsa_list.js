 ...
 const [dsaDetails, setDsaDetails] = useState({}); 
  ...
  const fetchDSADetails = async () => {
    try {
        ...
        const addresses = {};
        for (let i = 0; i < addressResponses.length; i++) {
            const dsaId = response.data[i].dsaId;
            addresses[dsaId] = addressResponses[i].data.permanentAddress;
        }
        setDsaAddress(addresses);

        // Fetch additional DSA details
        const dsaDetailsPromises = response.data.map(dsa => (
            axios.get(`http://148.251.230.14:8000/api/dsa`, {
                params: { dsaId: dsa.dsaId }
            })
        ));
        const dsaDetailsResponses = await Promise.all(dsaDetailsPromises);
        const details = {};
        dsaDetailsResponses.forEach((dsaDetailResponse, index) => {
            const dsaId = response.data[index].dsaId;
            details[dsaId] = dsaDetailResponse.data;
        });
        setDsaDetails(details);
    } catch (error) {
                ...
                {filteredDsas.length === 0 ? (
                  ...
                  filteredDsas.map((dsa, index) => (
                    <tr key={dsa._id}>
                      ...
                      <td>{dsa.dsaName}</td>
                      <td>{dsaDetails[dsa.dsaId]?.dsaCompanyName || 'N/A'}</td> Display DSA Company Name
